...
def parse_session_config(data: Dict[str, Any]) -> SessionConfig:
    # Get configuration from environment variables first, then fall back to metadata
    env_instructions = os.environ.get("INSTRUCTIONS")
    env_voice = os.environ.get("VOICE", "alloy")
    env_temperature = os.environ.get("TEMPERATURE")
    env_max_tokens = os.environ.get("MAX_OUTPUT_TOKENS")
    env_modalities = os.environ.get("MODALITIES")

    # Parse VAD settings from environment
    env_vad_threshold = os.environ.get("VAD_THRESHOLD")
    env_vad_silence_duration = os.environ.get("VAD_SILENCE_DURATION_MS")
    env_vad_prefix_padding = os.environ.get("VAD_PREFIX_PADDING_MS")

    # Build turn detection options
    turn_detection = None
    if data.get("turn_detection"):
        turn_detection_json = json.loads(data.get("turn_detection"))
        turn_detection = openai.realtime.ServerVadOptions(
            threshold=float(env_vad_threshold) if env_vad_threshold else turn_detection_json.get("threshold", 0.5),
            prefix_padding_ms=int(env_vad_prefix_padding) if env_vad_prefix_padding else turn_detection_json.get("prefix_padding_ms", 200),
            silence_duration_ms=int(env_vad_silence_duration) if env_vad_silence_duration else turn_detection_json.get("silence_duration_ms", 300),
        )
    else:
        # Use environment variables or defaults for turn detection
        turn_detection = openai.realtime.ServerVadOptions(
            threshold=float(env_vad_threshold) if env_vad_threshold else 0.5,
            prefix_padding_ms=int(env_vad_prefix_padding) if env_vad_prefix_padding else 300,
            silence_duration_ms=int(env_vad_silence_duration) if env_vad_silence_duration else 200,
        )

    # Parse temperature and max tokens
    temperature = float(env_temperature) if env_temperature else float(data.get("temperature", 0.8))

    if env_max_tokens:
        max_tokens = int(env_max_tokens) if env_max_tokens != "inf" else "inf"
    else:
        max_tokens = data.get("max_output_tokens") if data.get("max_output_tokens") == "inf" else int(data.get("max_output_tokens") or 2048)

    # Create config with environment variables as priority
    config = SessionConfig(
        openai_api_key=data.get("openai_api_key", ""),
        instructions=env_instructions if env_instructions else data.get("instructions", ""),
...
def run_multimodal_agent(ctx: JobContext, participant: rtc.Participant):
    ...
    logger.info("Configuration source information:")
    if os.environ.get("INSTRUCTIONS"):
        logger.info("Using INSTRUCTIONS from environment variable")
        logger.info(f"First 100 chars: {os.environ.get('INSTRUCTIONS')[:100]}...")
    if os.environ.get("VOICE"):
        logger.info(f"Using VOICE from environment: {os.environ.get('VOICE')}")
    if os.environ.get("TEMPERATURE"):
        logger.info(f"Using TEMPERATURE from environment: {os.environ.get('TEMPERATURE')}")
    if os.environ.get("MAX_OUTPUT_TOKENS"):
        logger.info(f"Using MAX_OUTPUT_TOKENS from environment: {os.environ.get('MAX_OUTPUT_TOKENS')}")
    if os.environ.get("MODALITIES"):
        logger.info(f"Using MODALITIES from environment: {os.environ.get('MODALITIES')}")
